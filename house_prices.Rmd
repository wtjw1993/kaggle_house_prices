...
library(caret) # split data into training and validation sets
...
```{r}
combined_noNA$MSSubClass <- NULL # removed due to differences between train and test sets
combined_noNA$Street <- NULL # removed because only 12 houses have gravel
combined_noNA$Heating <- NULL # remove Heating since a large majority of houses have gas heating
...
combined_noNA$Condition2 <- NULL # remove Condition2 since a large majority of houses are normal
...
train <- as_data_frame(cbind(combined_noNA[1:1460,], "SalePrice" = train$SalePrice))
test <- as_data_frame(combined_noNA[1461:2919,])
```
...
```{r}
train <- train %>% mutate(HouseAge = YrSold - YearBuilt, 
                        Remodeled = ifelse(YearRemodAdd == YearBuilt, "Yes", "No")) %>%
  select(-YrSold, -YearBuilt, -YearRemodAdd)
...
```{r}
takeLog <- c("SalePrice", "LotArea", "1stFlrSF", "GrLivArea")
takeLog1p <- c("2ndFlrSF", "LowQualFinSF", "WoodDeckSF", "OpenPorchSF", "EnclosedPorch", 
...
set.seed(42)
spl <- createDataPartition(train$Neighborhood)
training_set <- train[spl$Resample1,]
validation_set <- train[-spl$Resample1,]
```

Now, the bas.lm function will be used to perform BMA with Jeffreys-Zellner-Siow prior. With default parameters, the bas.lm function sampled 524288 models to produce the BMA model using Bayesian Adaptive Sampling (BAS).

```{r}
bma_model <- bas.lm(SalePrice ~ . - Id, data = training_set, prior = "BIC",
...
```{r}
bma_predict <- predict(bma_model, validation_set)
```



```{r}
test <- test %>% mutate(HouseAge = YrSold - YearBuilt, 
                        Remodeled = ifelse(YearRemodAdd == YearBuilt, "Yes", "No")) %>%
  select(-YrSold, -YearBuilt, -YearRemodAdd)
test$HouseAge[which(test$HouseAge < 0)] <- 0 # one house hasd an age of -1, replace with 0
takeLog <- c("LotArea", "1stFlrSF", "GrLivArea")
takeLog1p <- c("2ndFlrSF", "LowQualFinSF", "WoodDeckSF", "OpenPorchSF", "EnclosedPorch", 
               "3SsnPorch", "ScreenPorch")
takeSqrt <- c("LotFrontage", "BsmtFinSF1", "BsmtFinSF2", "BsmtUnfSF", "TotalBsmtSF", "HouseAge")
